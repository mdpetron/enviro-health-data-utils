...
nata_c_st <- nata_c %>% filter(State == mystate, County != "Entire State",
                               County != "Entire US") %>% select(Tract, FIPS,  `Total Cancer Risk (per million)`)
nata_r_st <- nata_r %>% filter(State == mystate, County != "Entire State",
                               County != "Entire US") %>% select(Tract, FIPS,  `Total Respiratory (hazard quotient)`)

#join
nata_st <- left_join(nata_c_st, nata_r_st)

nata_st$county = substr(nata_st$Tract, 6,11)
nata_st <- nata_st %>% filter(county == "000000")
...
#merge with NATA
mydata <- left_join(acsYear3, nata_st, by = c("GEOID" = "FIPS"))
...
smoke <- chr %>% select(fipscode, year, v009_rawvalue, v001_numerator, v001_denominator)
...
smoke <- smoke %>% select(GEOID, adultSmokeRate, prematureDeathRate,year)
...
mydata <- mydata %>% select(1,2,3,4,5,7,8,11,12,13)

#save to your computer for qgis step
write.csv(mydata, paste0("Data_County_", mystate, Sys.Date(), ".csv"))
